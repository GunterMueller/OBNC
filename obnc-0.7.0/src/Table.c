...
static void ReadFieldList(int isRoot, FILE *file, Trees_Node *resultPtr)
{
	Trees_Node field;
	int ch;
...
static void ReadRecord(int isRoot, FILE *file, Trees_Node *resultPtr)
{
	...
	if (ch == '(') {
		while (! AtEndOfList(file)) {
			ReadFieldList(isRoot, file, &fieldList);
	...
	*resultPtr = NULL;
	switch (symbol) {
		...
		case BOOLEAN_SYM:
			{
				...
				if (n == 1) {
					if (b) {
						*resultPtr = Trees_NewLeaf(TRUE);
						
					} else {
						*resultPtr = Trees_NewLeaf(FALSE);
					}
					Trees_SetType(Trees_NewLeaf(TREES_BOOLEAN_TYPE), *resultPtr);
				} else {
					PrintImportErrorPrefix();
		...
		case ARRAY_SYM:
				...
				if (length != NULL) {
					if (Trees_Symbol(length) == INTEGER) {
						if (Trees_Integer(length) < 0) {
							PrintImportErrorPrefix();
							fprintf(stderr, "error: negative array length: %d\n", Trees_Integer(length));
							exit(EXIT_FAILURE);
						}
					} else {
						PrintImportErrorPrefix();
			...
			break;
		case RECORD_SYM:
			ReadRecord(isRoot, file, resultPtr);
			break;
		case POINTER_SYM:
			{
...
static void ResolveTypes(const char identName[], void *identNode, void *symbolFileEntriesMap)
{
	...
	Maps_Map symbolFileEntries = symbolFileEntriesMap;
	...
	if (node == NULL) {
		...
		switch (Trees_Symbol(node)) {
			...
			case TREES_CHAR_CONSTANT:
				...
				break;
			case INTEGER:
				fprintf(file, "(%d %d)", INTEGER_SYM, Trees_Integer(node));
				break;
			case REAL:	
				fprintf(file, "(%d %.*g)", REAL_SYM, DBL_DIG, Trees_Real(node));
				break;
			case STRING:
				fprintf(file, "(%d \"%s\")", STRING_SYM, Trees_String(node));
				break;
			case TREES_SET_CONSTANT:	
				fprintf(file, "(%d %u)", SET_SYM, Trees_Set(node));
				break;
...
void Table_Export(const char filename[])
{
	...
	if ((Version_obnc != NULL) && (Version_obnc[0] != '\0')) {
		fprintf(exportFile, "%s", Version_obnc);
	}
	fputc('\n', exportFile);
