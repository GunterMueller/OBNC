...
OBNC_LONGI int stdArgs__count_;

void stdArgs__Get_(OBNC_LONGI int n, char arg[], OBNC_LONGI int argLen, OBNC_LONGI int *res)
{
	const char *srcArg;
	OBNC_LONGI int srcArgStrLen;

	assert(n >= 0);
	assert(n < stdArgs__count_);
	assert(arg != NULL);
	assert(argLen >= 0);
...
void stdArgs__Init(void)
{
	static int initialized = 0;

	if (! initialized) {
		stdArgs__count_ = Max(OBNC_argc - 1, 0);
		initialized = 1;
	}
}
