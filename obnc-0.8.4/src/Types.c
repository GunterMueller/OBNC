			...
			case ARRAY:
				if (Types_IsString(sourceType)) {
					...
					result = ! Types_IsOpenArray(targetType)
						&& Types_Same(Types_ElementType(sourceType), Types_ElementType(targetType));
				}								
				break;
			case RECORD:
...
static int AreEqualOpenArrays(Trees_Node typeA, Trees_Node typeB)
{
	return Types_IsOpenArray(typeA) && Types_IsOpenArray(typeB)
		&& Types_Same(Types_ElementType(typeA), Types_ElementType(typeB));
}
...
static int FormalParametersMatch(Trees_Node paramA, Trees_Node paramB)
{
	...
	return (kindA == kindB) && (Types_Same(typeA, typeB) || AreEqualOpenArrays(typeA, typeB));
}
