...
#define WITHIN_BOUNDS(x, y, plane) (((OBNC_LONGI unsigned int) (x) < (plane)->w) && ((OBNC_LONGI unsigned int) (y) < (plane)->h))

#define PIXEL_PTR(x, y, plane) (((Uint8 *) (plane)->pixels) + ((plane)->h - 1- (y)) * (plane)->w + (x))
		...
		if (useFullscreen) {
			...
			*height = info->current_h;
			...
			if (info->current_h > info->current_w) {
				nominalWidth = info->current_w * 4 / 5;
			} else {
				nominalWidth = info->current_h * 4 * 4 / (5 * 3);
			}
			*width = nominalWidth - nominalWidth % 4;
			*height = *width * 3 / 4;
		}
	} else {
...
{
	SDL_Surface *plane = SDL_GetVideoSurface();
...
void XYplane_Dot_(OBNC_LONGI int x, OBNC_LONGI int y, OBNC_LONGI int mode)
{
	...
	if ((plane != NULL) && WITHIN_BOUNDS(x, y, plane)) {
		*PIXEL_PTR(x, y, plane) = (mode == XYplane_draw_)? 0xff: 0;
	}
}
...
int XYplane_IsDot_(OBNC_LONGI int x, OBNC_LONGI int y)
{
	...
	return (plane != NULL) && WITHIN_BOUNDS(x, y, plane) && (*PIXEL_PTR(x, y, plane) == 0xff);
}


char XYplane_Key_(void)
{
	const int ack = 0x06; /*Ctrl-f*/
	const int esc = 0x1b;
	SDL_Surface *plane = SDL_GetVideoSurface();
	char result = 0;
	int fullscreen;
