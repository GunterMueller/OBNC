...
#endif
#ifdef OBNC_CONFIG_USE_LONG_REAL
	#define OBNC_ABS_FLT(x) OBNC_Abs(x)
#else
...
#define OBNC_ROR(x, n) OBNC_Ror(x, n)

/*Type conversions*/

#ifdef OBNC_CONFIG_USE_LONG_REAL
	#define OBNC_FLOOR(x) OBNC_Floor(x)
#else
...
#define OBNC_PACK(x, n) OBNC_Pack(&(x), n)
...
#define OBNC_DIV(x, y) OBNC_Div(x, y)

#define OBNC_MOD(x, y) OBNC_Mod(x, y)

#define OBNC_RANGE(m, n) OBNC_Range(m, n)
...
long double OBNC_Abs(long double x);

OBNC_LONGI int OBNC_Ror(OBNC_LONGI int x, OBNC_LONGI int n);

OBNC_LONGI int OBNC_Floor(long double x);
