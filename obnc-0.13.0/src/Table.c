...
{
	return IsLeadingIdentChar(ch) || ((ch >= '0') && (ch <= '9'));
}
...
{	
	static const struct { const char *name; int type; } procs[] = {
		...
		{"VAL", TREES_VAL_PROC}};
	char *name;
	size_t qualifierStrLen;
	int i;
	Trees_Node ident;

	qualifierStrLen = strlen(qualifier);
	for (i = 0; i < LEN(procs); i++) {
		NEW_ARRAY(name, qualifierStrLen + 32)
		sprintf(name, "%s.%s", qualifier, procs[i].name);
