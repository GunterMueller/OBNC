...
static void Append(const char extra[], char s[], size_t sLen)
{
	...
	sStrLen = (int) strlen(s);
	extraStrLen = strlen(extra);
	if (sStrLen + extraStrLen < sLen) {
		strcpy(s + sStrLen, extra);
	} else {
		fprintf(stderr, "target array too short for concatenation (length %lu): %s\n", sLen, s);
...
void Path_Get(const char module[], const char relativeDir[], char dirPath[], int dirPathLen)
{
	const char *impPaths;
	...
	dirPath[0] = '\0';
	Append(relativeDir, dirPath, dirPathLen);
		...
		if (impPaths != NULL) {
			...
			do {
				...
				if (pathEnd > pathStart) {
					memcpy(dirPath, impPaths + pathStart, (size_t) (pathEnd - pathStart));
