...
void Range_CheckIntSum(OBNC_LONGI int a, OBNC_LONGI int b)
{
	if ((b > 0) && (a > OBNC_INT_MAX - b)) {
		Oberon_PrintError("warning: integer overflow: %" OBNC_INT_MOD "d + %" OBNC_INT_MOD "d > %" OBNC_INT_MOD "d", a, b, OBNC_INT_MAX);
	} else if ((b < 0) && (a < OBNC_INT_MIN - b)) {
		Oberon_PrintError("warning: integer overflow: %" OBNC_INT_MOD "d + (%" OBNC_INT_MOD "d) < %" OBNC_INT_MOD "d", a, b, OBNC_INT_MIN);
	}
}
...
void Range_CheckFLOOR(OBNC_LONGR double x)
{
	if (x < (OBNC_LONGR double) INT_MIN) {
		Oberon_PrintError("warning: parameter in FLOOR too large for truncation to INTEGER: %" OBNC_REAL_MOD_W "E < %" OBNC_REAL_MOD_W "E", x, (OBNC_LONGR double) INT_MIN);
	} else if (x >= (OBNC_LONGR double) INT_MAX + 1.0) {
		Oberon_PrintError("warning: parameter in FLOOR too large for truncation to INTEGER: %" OBNC_REAL_MOD_W "E >= %" OBNC_REAL_MOD_W "E", x, (OBNC_LONGR double) INT_MAX + 1.0);
	}
}
