...
static const char *CurrentDirname(void)
{
	...
	if (result == NULL) {
		...
		if (p != NULL) {
			...
			fprintf(stderr, "obnc-compile: %s failed: %s\n", __FUNCTION__, strerror(errno));
			exit(EXIT_FAILURE);
		}
	}
	assert(result != NULL);
...
static void GenerateObjectFileSymbolDefinitions(Trees_Node identList, FILE *file, int indent)
{
	...
	if (strcmp(dirPrefix, "") != 0) {
		while (identList != NULL) {
			...
			Print(file, indent, "#define ");
			GenerateIdent(ident, file, 0);
			Print(file,  0, " %s", dirPrefix);
			GenerateIdent(ident, file, 0);
			Print(file,  0, "\n");
...
void Generate_ModuleHeading(void)
{
	...
	if (installPath != NULL) {	
		Print(cFile, 0, "%s\n\n", headerComment);
		if (hFile == NULL) {
			Print(cFile, 0, "#include \"%s/lib/obnc/OBNC_.h\"\n", installPath);
		} else {
			...
			Print(hFile, 0, "#include \"%s/lib/obnc/OBNC_.h\"\n", installPath);
		}
	} else {
		fprintf(stderr, "obnc-compile: undefined environment variable: OBNC_INSTALL_PATH\n");
		exit(EXIT_FAILURE);
	}
}
...
{
	Trees_Node moduleAndDirPath, module, dirPath;
	const char *dirPathStr, *parentDirPrefix;
	...
	while (list != NULL) {
		...
		module = Trees_Left(moduleAndDirPath);
		dirPath = Trees_Right(moduleAndDirPath);
		dirPathStr = Trees_String(dirPath);
		if (strcmp(dirPathStr, ".") == 0) {
			Print(cFile, 0, "#include \"%s.h\"\n", Trees_Name(module));
			if (hFile != NULL) {
				Print(hFile, 0, "#include \"%s.h\"\n", Trees_Name(module));
			}		
		} else {
			if ((dirPathStr[0] == '.') && (dirPathStr[1] == '/')) {
				parentDirPrefix = ".";
			} else {
				parentDirPrefix = "";
			}
			Print(cFile, 0, "#include \"%s%s/.obnc/%s.h\"\n", parentDirPrefix, dirPathStr, Trees_Name(module));
			if (hFile != NULL) {
				Print(hFile, 0, "#include \"%s%s/.obnc/%s.h\"\n", parentDirPrefix, dirPathStr, Trees_Name(module));
			}
		}
...
void Generate_ModuleStatements(Trees_Node stmtSeq)
{
	...
	if (Args_Mode() == ARGS_ENTRY_POINT_MODE) {
		...
		Print(cFile, 0, "{\n");
		Print(cFile, 1, "OBNC_Initialize(argc, argv);\n");
		if (importList != NULL) {
			GenerateInitCalls(1);
		}
		Generate(stmtSeq, cFile, 1);
		...
		sprintf(initFuncName, "%s_Init", Args_ModuleName());
		Print(cFile, 0, "\nvoid %s(void)\n", initFuncName);
		Print(cFile, 0, "{\n");
	...
	if (node != NULL) {
		...
		switch (symbol) {
			...
			case WHILE:
				...
				break;
			default:
				fprintf(stderr, "obnc-compile: %s failed: unknown symbol: %d\n", __FUNCTION__, Trees_Symbol(node));
				assert(0);
		}
	}
}
