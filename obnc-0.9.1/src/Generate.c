...
static int HasPointerOrProcedure(Trees_Node type)
{
	int result;
	Trees_Node fieldListSeq, fieldList, ident;
	
	result = 0;
	if (type != NULL) {
		switch (Trees_Symbol(Types_Structure(type))) {
			case	ARRAY:
				result = HasPointerOrProcedure(Types_ElementType(type));
				break;
			case RECORD:
				fieldListSeq = Types_Fields(type);
				while ((fieldListSeq != NULL) && ! result) {
					fieldList = Trees_Left(fieldListSeq);
					ident = Trees_Left(fieldList);
					if (HasPointerOrProcedure(Trees_Type(ident))) {
						result = 1;
					}
					fieldListSeq = Trees_Right(fieldListSeq);
				}
	...
	do {
		...
		if ((Trees_Kind(firstIdent) == TREES_VARIABLE_KIND) && Trees_Local(firstIdent) && (file != hFile)) {
			switch (Trees_Symbol(Types_Structure(Trees_Type(firstIdent)))) {
				case ARRAY:
				case RECORD:
					if (HasPointerOrProcedure(Trees_Type(firstIdent))) {
						Print(file, 0, " = {0}");
					}
					break;
				case POINTER:
...
{
	Print(cFile, 0, "%s\n\n", headerComment);
	Print(cFile, 0, "#include <obnc/OBNC.h>\n");
	if (hFile != NULL) {
		...
		Print(hFile, 0, "#include <obnc/OBNC.h>\n");
	}
}
...
void Generate_ImportList(Trees_Node list)
{
	...
	while (list != NULL) {
		...
		if (IsInstalledLibrary(dirPath)) {
			...
			Print(cFile, 0, "#include <%s/%s.h>\n", relativePath, Trees_Name(module));
			if (hFile != NULL) {
				Print(hFile, 0, "#include <%s/%s.h>\n", relativePath, Trees_Name(module));
			}
			...
			Print(cFile, 0, "#include \"%s%s/%s.h\"\n", parentDirPrefix, hFileDir, Trees_Name(module));
			if (hFile != NULL) {
				Print(hFile, 0, "#include \"%s%s/%s.h\"\n", parentDirPrefix, hFileDir, Trees_Name(module));
			}
		}
