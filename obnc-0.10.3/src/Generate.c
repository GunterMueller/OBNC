	...
	switch (Trees_Symbol(opNode)) {
		...
		case GE:
			...
			if (Trees_Symbol(leftType) == TREES_STRING_TYPE) {
				fprintf(file, "%lu", (long unsigned int) strlen(Trees_String(leftOperand)) + 1);
			} else {
			...
			if (Trees_Symbol(rightType) == TREES_STRING_TYPE) {
				fprintf(file, "%lu", (long unsigned int) strlen(Trees_String(rightOperand)) + 1);
			} else {
	...
	while (expList != NULL) {
		...
		if (isValueParam && Types_IsArray(fpType) && Types_IsArray(Types_ElementType(fpType))) {
			...
			if (Types_IsOpenArray(fpType)) {
				fprintf(file, "(const ");
				Generate(Types_ElementType(fpType), file, 0);
				fprintf(file, " *) ");
			} else {
				fprintf(file, "*(const ");
